...
class GitHubUserControllerIT {
  ...
  private void stubGitHub(String username) {
    ...
    WIREMOCK.stubFor(
        get("/users/" + username + "/repos?per_page=100").willReturn(okJson(REPOS_JSON)));
  }
  ...
  void happyPathReturnsCorrectJsonShape() {
    ...
    client
        ...
        .isEqualTo("The Octocat")
        .jsonPath("$.avatar")
        .isEqualTo("https://avatars.githubusercontent.com/u/583231?v=4")
        .jsonPath("$.geo_location")
        .isEqualTo("San Francisco")
        .jsonPath("$.url")
        .isEqualTo("https://api.github.com/users/octocat")
        .jsonPath("$.created_at")
        .isEqualTo("Tue, 25 Jan 2011 18:44:36 GMT")
        .jsonPath("$.repos[0].name")
        .isEqualTo("Hello-World")
        .jsonPath("$.repos[0].url")
  ...
  void unknownUserReturns404() {
    ...
    client.get().uri("/users/ghost").exchange().expectStatus().isNotFound();
  }
  ...
  void githubApiErrorReturns502() {
    ...
    WIREMOCK.stubFor(
        get("/users/broken/repos?per_page=100").willReturn(aResponse().withStatus(500)));

    client.get().uri("/users/broken").exchange().expectStatus().isEqualTo(502);
  }
