...
class CachedUserRepositoryTest {
  ...
  void savesAndRetrievesFreshEntry() {
    ...
    var found =
        repository.findFreshByUsername("octocat", OffsetDateTime.now(ZoneOffset.UTC).minusHours(1));
  ...
  void returnsEmptyForStaleEntry() {
    repository.save(
        new CachedUser("stale", SAMPLE, OffsetDateTime.now(ZoneOffset.UTC).minusHours(2)));

    var found =
        repository.findFreshByUsername("stale", OffsetDateTime.now(ZoneOffset.UTC).minusHours(1));
